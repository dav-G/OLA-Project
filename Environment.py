...
class ContextEnvironment():
	...
	def getReward(self, pulled_price, pulled_bid):
		...
		clicks = max(0,np.random.normal(self.customers[i].num_clicks(self.bids[pulled_bid]), self.sigmas[pulled_bid]))
		click_cost = np.random.normal(self.customers[i].click_cost(self.bids[pulled_bid]), self.sigmas[pulled_bid]/50)
	...
	def getDeterministicReward(self, pulled_price, pulled_bid):
		i = self.i
		
		clicks = self.customers[i].num_clicks(self.bids[pulled_bid])
		click_cost = self.customers[i].click_cost(self.bids[pulled_bid])
		
		sold = clicks * self.customers[i].conversion_probability(self.prices[pulled_price])
