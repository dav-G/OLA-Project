...
# %%
n_arms=10 #da cambiare bisogna mettere 100 utilizzare uno minure solo per vedere le convergenze
min_bid=0.0
max_bid=2.0
bids=np.linspace(min_bid,max_bid,n_arms)
sigma_num=15
sigma_cost=0.5
T=50 #horizon ricordarsi di cambiare prima del run finale perchè deve essere 365
n_experiment=3
best_price=11.66 #da cambiare vedere cosa esce da step 1
...
plt.plot(np.cumsum(np.mean(opt-gpucb_rewards_per_experiment, axis=0))+1.96*np.std(gpucb_rewards_per_experiment,axis=0),'--r')
plt.plot(np.cumsum(np.mean(opt-gpucb_rewards_per_experiment, axis=0))-1.96*np.std(gpucb_rewards_per_experiment,axis=0),'--r')
plt.plot(np.cumsum(np.mean(opt-gpts_rewards_per_experiment, axis=0))+1.96*np.std(gpts_rewards_per_experiment,axis=0),'--b')
plt.plot(np.cumsum(np.mean(opt-gpts_rewards_per_experiment, axis=0))-1.96*np.std(gpts_rewards_per_experiment,axis=0),'--b')
